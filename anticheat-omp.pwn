...
Basic AC By Martin - Open.MP Version
...
stock ac_KickPlayer(playerid, const reason[], Float:value = 0.0)
{
    ...
    KillTimer(ac_Data[playerid][ac_CheckTimer]);
    KillTimer(ac_Data[playerid][ac_ResetTimer]);
...
public OnPlayerSpawn(playerid)
{
    ...
    ac_Data[playerid][ac_SpawnTimer] = SetTimerEx("ac_ResetSpawnProtection", 3000, false, "i", playerid);
    ac_Data[playerid][ac_CheckTimer] = SetTimerEx("ac_CheckPlayer", CHECK_INTERVAL, true, "i", playerid);
    ac_Data[playerid][ac_ResetTimer] = SetTimerEx("ac_ResetWarnings", WARN_RESET_TIME, true, "i", playerid);
...
public OnPlayerConnect(playerid)
{
    ...
    ac_Data[playerid][ac_InVehicle] = false;
    ac_Data[playerid][ac_VehicleID] = INVALID_VEHICLE_ID;
    ac_Data[playerid][ac_CheckTimer] = 0;
    ac_Data[playerid][ac_ResetTimer] = 0;
    ac_Data[playerid][ac_SpawnTimer] = 0;
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    if(ac_Data[playerid][ac_CheckTimer] != 0)
    {
        KillTimer(ac_Data[playerid][ac_CheckTimer]);
        ac_Data[playerid][ac_CheckTimer] = 0;
    }
    if(ac_Data[playerid][ac_ResetTimer] != 0)
    {
        KillTimer(ac_Data[playerid][ac_ResetTimer]);
        ac_Data[playerid][ac_ResetTimer] = 0;
    }
    if(ac_Data[playerid][ac_SpawnTimer] != 0)
    {
        KillTimer(ac_Data[playerid][ac_SpawnTimer]);
        ac_Data[playerid][ac_SpawnTimer] = 0;
    }
...
public OnGameModeExit()
{
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        {
            if(ac_Data[i][ac_CheckTimer] != 0) KillTimer(ac_Data[i][ac_CheckTimer]);
            if(ac_Data[i][ac_ResetTimer] != 0) KillTimer(ac_Data[i][ac_ResetTimer]);
            if(ac_Data[i][ac_SpawnTimer] != 0) KillTimer(ac_Data[i][ac_SpawnTimer]);
        }
    }
    return 1;
}
